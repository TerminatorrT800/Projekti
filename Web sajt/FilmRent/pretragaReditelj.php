...
$pretragaIme = $_POST['ime'];
$pretragaPrezime = $_POST['prezime'];
$query = $conn->prepare("SELECT * FROM film 
          JOIN reditelj_film ON film.id = reditelj_film.film_id 
          JOIN reditelj ON reditelj_film.reditelj_id = reditelj.reditelj_id 
          WHERE reditelj.ime =? AND reditelj.prezime=?");
$query->bind_param('ss', $pretragaIme, $pretragaPrezime);
$query->execute();
