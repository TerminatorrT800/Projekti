...
$kategorija ="%". $_REQUEST['vrednost']."%";

$upit = $conn->prepare("SELECT * FROM film where kategorija LIKE ?");
$upit->bind_param("s", $kategorija);

if($upit->execute()){
    $rez = $upit->get_result();
        while($film = mysqli_fetch_assoc($rez)){
            echo '<div class="film">';
            echo '<img src="' . $film['slika'] . '" class="slika"/>';
            echo '<div>';
            echo '<a name="' . $film['id'] . '" class="naslov" href="prikazfilma.php?' . $film['id'] . '">' . $film['naziv'] . '</a>';
            echo '<div class="opis">';
            ...
            if ($glumcarez != false) {
                if (mysqli_num_rows($glumcarez) > 0) {
                    ...
                    while ($glumac = mysqli_fetch_assoc($glumcarez)) {
                        echo '<a class="godinaIzdanja" href="#">';
